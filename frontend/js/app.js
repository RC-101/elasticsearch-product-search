...
async function loadStatistics() {
    try {
        ...
        if (result.success) {
            ...
            document.getElementById('avgPrice').textContent = `₹${stats.price_stats.avg.toFixed(0)}`;
...
function displayResults(data, searchTime) {
    ...
    if (products.length === 0) {
        grid.innerHTML = '<div style="text-align:center; padding:40px; color:#6b7280;">No products found</div>';
        return;
    }
...
function updateAggregations(aggs) {
    ...
    categoryFilters.innerHTML = aggs.categories.slice(0, 10).map(cat => `
        <label class="checkbox-item">
            <input type="checkbox" value="${cat.key}" class="category-filter">
            ${cat.key}
    ...
    brandFilters.innerHTML = aggs.brands.slice(0, 10).map(brand => `
        <label class="checkbox-item">
            <input type="checkbox" value="${brand.key}" class="brand-filter">
            ${brand.key}
    ...
    priceRangeFilters.innerHTML = aggs.price_ranges.map(range => `
        <div class="info-item">
            <span>${range.key}</span>
    ...
    tagFilters.innerHTML = aggs.tags.map(tag => `
        <span class="tag-item" data-tag="${tag.key}">${tag.key} (${tag.doc_count})</span>
    `).join('');
...
function applyFilters() {
    ...
    if (selectedCategories.length > 0) {
        currentFilters.category = selectedCategories[0]; // API supports single category
    } else {
    ...
    if (selectedBrands.length > 0) {
        currentFilters.brand = selectedBrands[0]; // API supports single brand
    } else {
        delete currentFilters.brand;
    }
    
    // Get selected tags
    const selectedTags = Array.from(document.querySelectorAll('.tag-item.active'))
...
async function fetchAutocomplete(query) {
    try {
        const response = await fetch(`${API_BASE_URL}/autocomplete?q=${encodeURIComponent(query)}&size=8`);
        const result = await response.json();
...
async function showProductDetail(productId) {
    try {
        ...
        if (result.success) {
            ...
            const similarResponse = await fetch(`${API_BASE_URL}/products/${productId}/similar?size=6`);
            const similarResult = await similarResponse.json();
            
            const modalBody = document.getElementById('modalBody');
            modalBody.innerHTML = createProductDetail(product, similarResult.similar_products || []);
...
function createProductDetail(product, similarProducts) {
    ...
    const similarHtml = similarProducts.length > 0 ? `
